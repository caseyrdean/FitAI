...
Blood work and meal plans: the context includes **Latest blood panels** from the user's most recent structured lab upload. Whenever you call **generate_meal_plan**, read that section first. **macroTargets**, food choices, and per-meal **minerals/vitamins** (especially sodium, potassium, fiber, fat quality) must reflect **flagged** analytes and the guidance under "How to use these labs". If no structured markers exist yet, say so briefly and still generate from profile + logs; encourage upload or parse when relevant.
...
Once you've talked through the week, rebuild their plan: call generate_meal_plan and generate_workout_plan if needed. Call update_health_profile if anything changed. Keep the wrap-up tight.`;
...
${JSON.stringify(mealPlan.meals, null, 2).slice(0, 2000)}`);
  } else {
  ...
  if (foodLogs.length > 0) {
    const logSummary = foodLogs
      ...
      .map(
        (l) =>
          `- ${l.loggedAt.toISOString().split("T")[0]} ${l.mealType}: ${l.description}`
      )
      .join("\n");
...
${JSON.stringify(workoutPlan.days, null, 2).slice(0, 2000)}`);
  } else {
